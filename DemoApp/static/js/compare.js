...
//   let accuracyChartInstance = null; // THAY ĐỔI: Tên biến và mục đích rõ ràng hơn
...
//       // THAY ĐỔI: Logic hủy biểu đồ cũ
//       if (accuracyChartInstance) {
//         accuracyChartInstance.destroy();
//         accuracyChartInstance = null; 
//       }

//       // THAY ĐỔI: Gọi hàm chỉ để tạo DOM, không truyền biểu đồ cũ
//       displayComparisonResults(data, chartContainer); // Tên hàm mới, không truyền accuracyChartInstance

//       // THAY ĐỔI: Tạo biểu đồ mới và lưu instance
//       const chartObjects = createComparisonCharts(data); // Không cần truyền chartContainer nữa nếu tìm bằng ID
//       if (chartObjects && chartObjects.accuracyChart) {
//         accuracyChartInstance = chartObjects.accuracyChart; 
//       }
//     })
...
function initCompareForm() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    .then(data => {
      ...
      comparisonChart = createComparisonCharts(data, chartContainer); // Gọi sau khi container đã được làm mới
    })
    .catch(error => {
      handleFormError(error, 'compare-form');
    });
...
//   // }
//   // Clear existing chart if any
//   if (existingChart && existingChart.accuracyChart) {
//     existingChart.accuracyChart.destroy(); // Hủy biểu đồ cụ thể
//   }
...
function displayComparisonResults(results, container, existingChart) {
  ...
  // Clear existing chart if any
  if (existingChart && existingChart.accuracyChart) {
    existingChart.accuracyChart.destroy();
  }
